...
$("#search_section").html(load_str);  
   ...
   var dep_select_id_array=dep_select_array[1].split("_");
	     ...
	     if(START_DATE_PICKER_values.length>0 && END_DATE_PICKER_values.length>0){
	    	...
	    	var query=	"select DATE_FORMAT(todo_list.BTDL_CREATED_TIME,'%d/%m/%Y') ,"+
	    	  " todo_list.btdl_owner,"+
	    	  " count(*) ,"+
	    	  " concat(user_outer.firstname,' ',user_outer.lastname) ,"+
	    	  " DATE_FORMAT(todo_list.BTDL_CREATED_TIME,'%Y-%m-%d') "+
	    	  ...
	    	  " and  exists (SELECT username FROM "+SCHEMA_G+".BPM_DEPARTMENT_USER dept_user left join "+SCHEMA_G+".user u  "+
	    	  "  on dept_user.USER_ID=u.id where "+BDEPT_ID_QUERY+" "+
	    	 
	    	  ")    "+
	    	  "  group by todo_list.btdl_owner , DATE_FORMAT(todo_list.BTDL_CREATED_TIME,'%d/%m/%Y')  "+
	    		" order by todo_list.BTDL_CREATED_TIME, todo_list.btdl_owner asc";
	    		
	    	var query_finish ="select DATE_FORMAT(todo_list.BTDL_CREATED_TIME,'%d/%m/%Y') ,"+
	    	  " todo_list.btdl_owner,"+
	    	  " count(*) ,"+
	    	  " concat(user_outer.firstname,' ',user_outer.lastname) ,"+
	    	  " DATE_FORMAT(todo_list.BTDL_CREATED_TIME,'%Y-%m-%d') "+
	    	  ...
	    	  " and todo_list.btdl_hide='0' and  exists (SELECT username FROM "+SCHEMA_G+".BPM_DEPARTMENT_USER dept_user left join "+SCHEMA_G+".user u  "+
	    	  "  on dept_user.USER_ID=u.id where "+BDEPT_ID_QUERY+" "+
	    	...
	    	var query_pending ="select DATE_FORMAT(todo_list.BTDL_CREATED_TIME,'%d/%m/%Y') ,"+
	    	  " todo_list.btdl_owner,"+
	    	  " count(*) ,"+
	    	  " concat(user_outer.firstname,' ',user_outer.lastname) ,"+
	    	  " DATE_FORMAT(todo_list.BTDL_CREATED_TIME,'%Y-%m-%d') "+
	    	...
	    	var data_finish=null;
	    	SynDomeBPMAjax.searchObject(query_finish,{ 
	    	...
	    	var data_pending=null;
	    	SynDomeBPMAjax.searchObject(query_pending,{ 
				callback:function(data){
					if(data.resultMessage.msgCode=='ok'){
						data=data.resultListObj;
						data_pending=data;
	    	   ...
	    	   SynDomeBPMAjax.searchObject(query,{ 
					//SynDomeBPMAjax.searchObject(queryObject,{
						callback:function(data){
					        ...
					        "	<thead> 	"+
					        "  		<tr> "+
					        "  		<th width=\"7%\"><div class=\"th_class\">วันที่ได้รับงาน</div></th> "+
					        "		<th width=\"10%\"><div class=\"th_class\">ผู้บฏิบัติงาน</div></th> "+
					        "  		<th width=\"7%\"><div class=\"th_class\">ทั้งหมด</div></th> "+
					        "  		<th width=\"7%\"><div class=\"th_class\">เสร็จ</div></th> "+
					        "  		<th width=\"7%\"><div class=\"th_class\">ค้าง</div></th> "+
					        " 		</tr>"+
					        "	</thead>"+
					        "	<tbody>   ";  
							 var name_compare="";
							 ...
							 var date_compare=""
							   ...
							   if(data!=null && data.length>0){
								   ...
								   for(var i=0;i<data_size;i++){ 
									   ...
									   sum=sum+data[i][2];
									   if(date_compare==data[i][0]){
										   date_show=""; 
										   
									   } 
									  
									   if(date_compare!=data[i][0] && i!=0){
										   sum=sum-data[i][2];
									        str=str+"<tr>"+
										           "  <th><div class=\"th_class\"></div></th>"+
												   "  		<th style=\"text-align: left;background-color:#DCDF9C\"><div class=\"th_class\">รวม</div></th>"+    
												   "  		<th style=\"text-align: center;background-color:#DCDF9C\"><div class=\"th_class\"><span>"+sum+"</span></div></th>";
												  // "  		<th style=\"text-align: center;background-color:#DCDF9C\"><div class=\"th_class\"><span>"+sum_finish+"</span></div></th>";
											        str=str+ "</tr>"; 
											 sum=0;
											 sum=sum+data[i][2];
											// sum_finish=0;
									   ...
									   str=str+ "  	<tr >"+
									   "  <td style=\"text-align: center;\">"+date_show+"</td>"+    
								       "  <td style=\"text-align: left;\"><span>"+name_show+"</span> </td>"+
									   "  <td style=\"text-align: center;\"><span style=\"text-decoration: underline;cursor: pointer;\" onclick=\"showPopupDetail('"+data[i][1]+"','"+data[i][4]+"','"+data[i][4]+"','','-1')\">"+data[i][2]+"</span></td>";
									   //-----------------for finish job-------------------
	    
									  
									    if(data_finish!=null && data_finish.length>0){
									    	var data_finish_size=data_finish.length; 
							
											   for(var j=0;j<data_finish_size;j++){ 		
												    sum_finish=sum_finish+data_finish[j][2];
												    var strCompare = data_finish[j][1]+"_"+data_finish[j][4];
													   if(data_finish[j][0]==data[i][0] && data_finish[j][1]==data[i][1]){						    
									    				 str=str+ "  <td style=\"text-align: center;\"><span style=\"text-decoration: underline;cursor: pointer;\" onclick=\"showPopupDetail('"+data[i][1]+"','"+data[i][4]+"','"+data[i][4]+"','','0')\">"+data_finish[j][2]+"</span> </td> ";
									    			}
											   }
											   
									    }else{
									    	str=str+ "  <td style=\"text-align: center;\"><span>0</span> </td> ";
									    }
									   //-----------------for finish job-------------------
									 
									   if(data_pending!=null && data_pending.length>0){
						
									    	var data_pending_size=data_pending.length; 
									   
											   for(var k=0;k<data_pending_size;k++){ 		
												    sum_pending=sum_pending+data_pending[k][2];
												    var strCompare = data_pending[k][1]+"_"+data_pending[k][4];
													   if(data_pending[k][0]==data[i][0] && data_pending[k][1]==data[i][1]){						    
									    				 str=str+ "  <td style=\"text-align: center;\"><span style=\"text-decoration: underline;cursor: pointer;\" onclick=\"showPopupDetail('"+data[i][1]+"','"+data[i][4]+"','"+data[i][4]+"','','1')\">"+data_pending[k][2]+"</span> </td> ";
									    			}
											   }
									    }else{
									    	str=str+ "  <td style=\"text-align: center;\"><span>0</span> </td> ";
...
function showPopupDetail_sumAll(){
  ...
  showPopupDetail('',created_time_start,created_time_end,dep_id,'-1');
}
function showPopupDetail_sum(btdl_owner){
	   ...
	   var created_time_start= START_DATE_PICKER_array[2]+"-"+START_DATE_PICKER_array[1]+"-"+START_DATE_PICKER_array[0];
	   var created_time_end=END_DATE_PICKER_array[2]+"-"+END_DATE_PICKER_array[1]+"-"+END_DATE_PICKER_array[0];
	   showPopupDetail(btdl_owner,created_time_start,created_time_end,'','-1');
}
function showPopupDetail(btdl_owner,created_time_start,created_time_end,dept_id,isActive){
	...
	if(isActive!='-1'){
		query_isactive = " and todo_list.btdl_hide='"+isActive+"' ";
	}
	var query=" SELECT "+
	  ...
	  " IFNULL(call_center.BCC_SERIAL,'')  as c1,"+
	    ...
	    " IFNULL(DATE_FORMAT(call_center.BCC_DUE_DATE_START,'%H:%i'),'') as c12 , "+
	 	...
	 	" IFNULL(DATE_FORMAT(call_center.BCC_CREATED_TIME,'%d/%m/%Y'),'') as c14  "+

	 	" from "+SCHEMA_G+".BPM_TO_DO_LIST todo_list   "+
	 	" left join "+SCHEMA_G+".BPM_CALL_CENTER call_center on (call_center.bcc_no=todo_list.BTDL_REF  and todo_list.btdl_type=2) "+
	 	" where    todo_list.BTDL_CREATED_TIME between '"+created_time_start+" 00:00:00' and '"+created_time_end+" 23:59:59'  "+
	 	query_owner+query_isactive+
	 	" "+query_dept+
	    "order by  call_center.BCC_NO asc ";
	...
	SynDomeBPMAjax.searchObject(query,{
		callback:function(data){
			...
			var str="	  <table class=\"table table-striped table-bordered table-condensed\" border=\"1\" style=\"font-size: 12px;\"> "+
			        ...
			        "  		<th width=\"10%\"><div class=\"th_class\">วันที่เปิด Job</div></th> "+
			        "  		<th width=\"15%\"><div class=\"th_class\">หมายเลขเครื่อง/Model</div></th> "+ 
			        "  		<th width=\"58%\"><div class=\"th_class\">ชื่อลูกค้า/ที่อยู่</div></th> "+
			   ...
			   if(data!=null && data.length>0){
				   for(var i=0;i<data.length;i++){  
				       str=str+ "  	<tr style=\"cursor: pointer;\">"+  
						   ...
						   "  		<td style=\"text-align: left;\">"+data[i][14]+"</span></td>"+     
