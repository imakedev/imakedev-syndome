...
$(document).ready(function() { 
	$("#bsoTypeNo").keypress(function(event) {
		  if ( event.which == 13 ) {
...
function getTodolist(_page){  
	...
	//$("#pageNo").val(_page); 
	var bsoTypeNo = jQuery.trim($("#bsoTypeNo").val().replace(/'/g,"''"));
	var bsoTypeNoQuery='';
	if(bsoTypeNo.length>0)
		bsoTypeNoQuery =" and todo.REF_NO like '%"+bsoTypeNo+"%' "; 
		...
		" OR ( todo.BTDL_OWNER='"+usernameG+"' and todo.BTDL_OWNER_TYPE='1' ) )  and todo.BTDL_HIDE='1' "+bsoTypeNoQuery;
	...
	SynDomeBPMAjax.searchObject(queryObject,{
		callback:function(data){
			...
			var str="	  <table class=\"table table-striped table-bordered table-condensed\" border=\"1\" style=\"font-size: 12px\"> "+
			        ...
			        "  		<th width=\"56%\"><div class=\"th_class\">Task</div></th> "+
			        "  		<th width=\"12%\"><div class=\"th_class\">Process Name</div></th> "+
			        "  		<th width=\"10%\"><div class=\"th_class\">Last Updated</div></th> "+
			        "  		<th width=\"10%\"><div class=\"th_class\">จัดของครั้งที่</div></th> "+ 
			        " 		</tr>"+
			        "	</thead>"+
			        "	<tbody>   ";  
			   if(data!=null && data.length>0){
				   for(var i=0;i<data.length;i++){
					  ...
					  // var last_count = (data[i][16]!=null && data[i][16].length>0)? $.format.date(data[i][16], "dd/MM/yyyy  HH:mm"):"&nbsp;";
					  /*  alert(data[i][17])
					   alert(data[i][16]) */
					 /*  var sla="";
					  if(data[i][13]==60)
					  ...
					  var sla_alert="";
					   ...
					   "  		<td style=\"text-align: left;\"> "+data[i][6]+" </td>"+    
				        ...
				        "    	<td style=\"text-align: center;\">"+data[i][16]+"</td>  "+  
				        "  	</tr>  ";
				   }
			   }else{
	    	...
	    	"<tbody>"; 
			        ...
			        str=str+  " </tbody>"+
					   "</table> "; 
			$("#search_section_prepare").html(str);
		}
	}); 
	/*
            ...
            <input type="hidden" id="pageCount" value="1"/>
			   ...
			   <div style="border: 1px solid #FFC299;background: #F9F9F9;padding: 10px"> 
	    					...
	    					<td align="left" width="70%">   
	    					 <span><strong>รายการที่จัดแล้ว</strong></span>
	    					...
	    					<tr>
	    					<td align="left" width="70%">   
	    					 <span><strong>Ref No.</strong>
	    					  <input type="text" id="bsoTypeNo" style="height: 30px;width: 125px"/>
	    					</span>
	    					<span style="padding-left: 20px"><a style="margin-top:-10px" class="btn" onclick="getTodolist('1')">Search</a> </span>
	    					</td><td align="right" width="30%"> 
