...
function doAction(id){
	var querys=[]; 
	var query="DELETE FROM "+SCHEMA_G+".BPM_PROBLEM where BPR_ID="+id; 
	querys.push(query);
	//alert(query);
	//  return false;
...
function searchProblem(_page){ 
	/* var pdName =jQuery.trim($("#pdName").val());
	if(pdName.length>0)
		pdNameWhere=" where lower(pd_name) like lower('%"+pdName+"%')"; */
	$("#pageNo").val(_page);
//	var query_count="  SELECT (select count(*) from "+SCHEMA_G+".BPM_Problem  sla_count ";
	var query="SELECT BPR_ID,BPR_NAME  from "+SCHEMA_G+".BPM_PROBLEM "; 
	...
	SynDomeBPMAjax.searchObject(queryObject,{
		callback:function(data){
			...
			var str="	  <table class=\"table table-striped table-bordered table-condensed\" border=\"1\" style=\"font-size: 12px\"> "+
			        ...
			        "  		<th width=\"87%\"><div class=\"th_class\">ประเภทปัญหา</div></th> "+ 
			   ...
			   if(data!=null && data.length>0){
				   for(var i=0;i<data.length;i++){
					   ...
					   "  		<td style=\"text-align: left;\"> "+data[i][1]+" </td>"+    
				        ...
				        "    	<i title=\"Edit\" onclick=\"loadDynamicPage('setting/page/problemType_management?bprId="+data[i][0]+"&mode=edit')\" style=\"cursor: pointer;\" class=\"icon-edit\"></i>&nbsp;&nbsp;"+
	    	...
	    	"<tbody>"; 
			        ...
			        str=str+  " </tbody>"+
					   "</table> "; 
			$("#search_section_problemtype").html(str);
		}
	}); 
...
<div id="dialog-confirmDelete" title="Delete Problem" style="display: none;background: ('images/ui-bg_highlight-soft_75_cccccc_1x100.png') repeat-x scroll 50% 50% rgb(204, 204, 204)">
           ...
           <input type="hidden" id="pageNo" value="1"/>
            ...
            <input type="hidden" id="pageCount" value="1"/>
			   ...
			   <div style="border: 1px solid #FFC299;background: #F9F9F9;padding: 10px"> 
	    					...
	    					<span><strong>ประเภทปัญหา</strong></span>
	    					</td><td align="right" width="30%"> 
	    					</td>
	    					</tr>
	    					<tr>
	    					<td align="left" width="70%">   
	    					<a class="btn btn-primary"  onclick="loadDynamicPage('setting/page/problemType_management?bprId=0&mode=add')"><i class="icon-plus-sign icon-white"></i>&nbsp;<span style="font-weight:bold;color:  white;">Add</span></a>
	    					</td><td align="right" width="30%"> 
	    					...
	    					<div  id="search_section_problemtype"> 
    						</div>
