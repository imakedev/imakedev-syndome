			   ...
			   if(data!=null && data.length>0){
				   ...
				   for(var i=0;i<data.length;i++){ 
					   ...
					   "  		<td style=\"text-align: left;\"> "+serial+" </td>"+   
				        ...
				        "	<i title=\"Edit\" onclick=\"loadDynamicPage('dispatcher/page/delivery_install_preview?bsoId=${bsoId}&mode=edit&state=${state}&requestor=${requestor}&ima_itemid="+data[i][0]+"&serial="+data[i][2]+"')\" style=\"cursor: pointer;\" class=\"icon-edit\"></i>&nbsp;&nbsp;"+
				        "</td>  "+  
				        "  	</tr>  ";
				   }
 ...
 function doCloseJob(btdl_type,btdl_state,owner,owner_type,message){
	   /*  alert($("#BSO_IS_DELIVERY").val());
	    alert($("#BSO_IS_INSTALLATION").val()) */; 
	    	...
	    	var querys=[]; 
			var btdl_state_update='${state}'; 
			var query="update "+SCHEMA_G+".BPM_TO_DO_LIST set BTDL_HIDE='0' ,BTDL_ACTION_TIME=now() where BTDL_REF='${bccNo}' and "+
			"BTDL_TYPE='"+btdl_type+"' and BTDL_STATE='"+btdl_state_update+"' and BTDL_OWNER='"+owner+"' and BTDL_OWNER_TYPE='"+owner_type+"'  ";	 
			//alert(query)
			querys.push(query); 
			SynDomeBPMAjax.executeQuery(querys,{
					callback:function(data){ 
						...
						if(data!=0){ 
							 querys=[];   
						 if(btdl_state!='wait_for_create_to_express' && btdl_state!='wait_for_stock'){
								 query="update "+SCHEMA_G+".BPM_SERVICE_JOB set BSJ_STATE='"+btdl_state+"' where BCC_NO=${bccNo}"; 
							querys.push(query); 
							SynDomeBPMAjax.executeQuery(querys,{
								callback:function(data){ 
									if(data.resultMessage.msgCode=='ok'){
...
<fieldset style="font-family: sans-serif;padding-top:5px">
    <div style="border: 1px solid #FFC299;background: #F9F9F9;padding: 10px">
	    <form id="breakdownForm" name="breakdownForm"  class="well" action="" method="post">
			...
			<c:if test="${isSupervisorAccount && ( state=='wait_for_supervisor_services_close' )}">
				<div align="center" style="padding-top: 10px">
				 <table border="0" style="width: 100%">
				 	<tr>
				 		<td width="20%">
				 		...
				 		<td width="80%"> 
				 		 <div align="center"> <a class="btn btn-primary"  onclick="doCloseJob('2','wait_for_supervisor_services_close','${username}','1','Job Closed')"><i class="icon-ok icon-white"></i>&nbsp;<span style="color: white;font-weight: bold;">Close Job</span></a></div>
