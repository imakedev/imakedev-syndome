...
function initPMMA(){
   ...
   var query="SELECT user.id,user.username,user.firstName,user.lastName ,user.email,user.mobile ,user.detail "+
	" FROM "+SCHEMA_G+".BPM_DEPARTMENT_USER  dept_user  left join  "+SCHEMA_G+".user on dept_user.USER_ID=user.id "+ 
	" where dept_user.BDEPT_ID=10";
SynDomeBPMAjax.searchObject(query,{
	callback:function(data){
		...
		var str="	 <select name=\"pmma_center\" id=\"pmma_center\" onchange=\"searchPMMA('1')\" style=\"width:250px\"> "+
	...
	*/
	...
	var query_where="";
	 ...
	 if(pmma_jobno_search.length>0){
			//query_where=query_where+" and pmma.BPMJ_SO_NO like '%"+pmma_jobno_search+"%'";
			query_where=query_where+" and concat(ifnull(pmma.BPMJ_SO_NO,''),IFNULL(BPMJ_SERAIL,'')) like '%"+pmma_jobno_search+"%'";
		}
	 
	var bpmj_job_status=$("#pmma_jobstatus_search").val();
	/*
	if(pmma_center!='0'){
	...
	"  left join "+SCHEMA_G+".BPM_CENTER center on center.uid=user.id "+ 
	 " 	where pmma.bpmj_job_status="+bpmj_job_status+"  "+query_where+
	...
	SynDomeBPMAjax.searchObject(queryObject,{
		callback:function(data){
			...
			var str="	  <table class=\"table table-striped table-bordered table-condensed\" border=\"1\" style=\"font-size: 12px\"> "+
			        ...
			        "  		<th width=\"24%\"><div class=\"th_class\">หมายเลขเครื่อง/Model</div></th> "+ 
			   ...
			   if(data!=null && data.length>0){
				   for(var i=0;i<data.length;i++){
					   ...
					   "    	<td style=\"text-align: left;\">"+data[i][3]+"</td>  "+  
				        ...
				        "    	<td style=\"text-align: left;\"><span>"+data[i][11]+"</span></td>  "+
	    	...
	    	"<tbody>"; 
			        ...
			        str=str+  " </tbody>"+
					   "</table> "; 
			$("#search_section_pmma_complete").html(str);
		}
	}); 
	/*
           ...
           <input type="hidden" id="pageNo" value="1"/>
            ...
            <input type="hidden" id="pageCount" value="1"/>
			   ...
			   <div style="border: 1px solid #FFC299;background: #F9F9F9;padding: 10px"> 
	    					...
	    					<td align="left" colspan="2" width="100%">   
	    					 ...
	    					 <span style="padding-left:5px">
	    					  ...
	    					  <span style="padding-left:5px"> 
							    ...
							    --%>
							       <span style="padding-left:20px"><strong>Job Status :</strong></span>
	    					  <span style="padding-left:5px"> 
	    					  <select name="pmma_jobstatus_search" id="pmma_jobstatus_search" onchange="searchPMMA('1')" style="width: 150px">
	    					  	<option value="3">ตรวจสอบเอกสาร</option>
	    					  	<option value="4">ปิดงานเรียบร้อย</option> 
	    					  </select>
							   </span>  
							     <span style="padding-left:20px"><strong>SO / Serial  No:</strong></span>
	    					  <span style="padding-left:5px"> 
	    					...
	    					<div  id="search_section_pmma_complete"> 
    						</div> 
      </div>
      </fieldset> 