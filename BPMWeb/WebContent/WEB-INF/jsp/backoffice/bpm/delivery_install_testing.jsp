   ...
   $("#BSO_RFE_DATE_PICKER" ).datepicker({
	...
	/*  $("#BSO_DELIVERY_DUE_DATE_TIME_PICKER" ).timepicker({
	 ...
	 $('#BSO_RFE_DATE_TIME_PICKER').timepicker({
		...
		new AjaxUpload('BIT_DOC_ATTACH', {
			  action: 'upload/Installation/${bsoId}_${ima_itemid}_${serial}',
			...
			onComplete : function(file, response){ 
				...
				$("#BIT_DOC_ATTACH_SRC").html(obj.filename);
			     $("#BIT_DOC_ATTACH_SRC").attr("onclick","loadFile('getfile/Installation/${bsoId}_${ima_itemid}_${serial}/"+obj.hotlink+"')");
			}		
		}); 
		...
		BIT_ATTACH_6 */
	for(var i=1 ;i<=6;i++){
		new AjaxUpload('BIT_ATTACH_'+i, {
			action: 'upload/Installation/${bsoId}_${ima_itemid}_${serial}_'+i,
			onSubmit : function(file , ext){
	            // Allow only images. You should add security check on the server-side.
				if (ext && /^(jpg|png|jpeg|gif|xls|xlsx|XLS|XLSX|pdf|PDF|docx|doc|DOCX|DOC)$/.test(ext)){
					/* Setting data */
			...
			onComplete : function(file, response){ 
				var obj = jQuery.parseJSON(response);
			//	alert(obj.filename)
				$("#BIT_ATTACH_"+obj.id+"_SRC").attr("style","text-decoration: underline;cursor:pointer"); 
				$("#BIT_ATTACH_"+obj.id+"_SRC").html(obj.filename);
			     $("#BIT_ATTACH_"+obj.id+"_SRC").attr("onclick","loadFile('getfile/Installation/${bsoId}_${ima_itemid}_${serial}_"+obj.id+"/"+obj.hotlink+"')");
			}		
		}); 
	}
...
function doCloseInstallJob(btdl_type,btdl_state,owner,owner_type,message_duplicate,message_created,message_todolist,hide_status,is_hide_todolist){   
	var BIT_CUST_NAME=jQuery.trim($("#BIT_CUST_NAME").val()); 
	if(!BIT_CUST_NAME.length>0){
			  alert("กรุณาใส่ ชื่อลูกค้าตรวจสอบการติดตั้ง");
			  $("#BIT_CUST_NAME").focus();
			  return false;
	}
	var BIT_DOC_ATTACH_SRC =jQuery.trim($("#BIT_DOC_ATTACH_SRC").html());
	//alert(BDT_DOC_ATTACH_SRC.length)
	if(!BIT_DOC_ATTACH_SRC.length>0){
		  alert("กรุณา แนบเอกสาร");
		  $("#BIT_DOC_ATTACH").focus();
		  return false;
   }
	 ...
	 SynDomeBPMAjax.searchObject(query,{
			callback:function(data){ 
				...
				if(data!=null && data.length>0){
					...
					var querys=[];  
					var BIT_TYPE=$("#BIT_TYPE").val();
					var BIT_LOAD=$("#BIT_LOAD").val();
					var BIT_BACKUP_TIME=$("#BIT_BACKUP_TIME").val();
					var BIT_COMMENT=$("#BIT_COMMENT").val();
				//	alert(BIT_COMMENT)
					var BIT_TEST_INPUT_query="";
					var BIT_TEST_OUTPUT_query="";
					for(var i=1;i<=7;i++){ 
						var BIT_TEST_INPUT=jQuery.trim($("#BIT_TEST_INPUT_"+i).val());
						var BIT_TEST_OUTPUT=jQuery.trim($("#BIT_TEST_OUTPUT_"+i).val()); 
						BIT_TEST_INPUT_query=BIT_TEST_INPUT_query+" BIT_TEST_INPUT_"+i+"='"+BIT_TEST_INPUT+"'," ;
						BIT_TEST_OUTPUT_query=BIT_TEST_OUTPUT_query+" BIT_TEST_OUTPUT_"+i+"='"+BIT_TEST_OUTPUT+"'," ;
					}
					var query_update=" UPDATE "+SCHEMA_G+".BPM_INSTALLATION_TRAN SET "+ 
					" BIT_CUST_NAME = '"+BIT_CUST_NAME+"' , "+ 
					BIT_TEST_INPUT_query+BIT_TEST_OUTPUT_query+
					"BIT_LOAD='"+BIT_LOAD+"' , BIT_BACKUP_TIME='"+BIT_BACKUP_TIME+"',BIT_TYPE='"+BIT_TYPE+"',BIT_COMMENT='"+BIT_COMMENT+"' , BIT_CHECK_STATUS='1' "+
					"where  BSO_ID=${bsoId} and IMA_ItemID='${ima_itemid}' and "+
					"  SERIAL='${serial}' ";
					//alert(query_update)
				 ...
				 //return false;
					...
					SynDomeBPMAjax.executeQuery(querys,{
						callback:function(data){ 
							...
							if(data!=0){ 
								 loadDynamicPage('dispatcher/page/delivery_operation?bsoId=${bsoId}&mode=edit&state=${state}&requestor=${requestor}');
    ...
    <div style="border: 1px solid #FFC299;background: #F9F9F9;padding: 10px">
	    <form id="breakdownForm" name="breakdownForm"  class="well" action="" method="post">
          ...
          <!--   <input type="hidden" id="LastCostAmt" name="LastCostAmt"/> -->
    				...
    				<tr valign="top">
    					...
    					<div  id="cust_input_1" style="border: 1px solid #FFC299;background: #F9F9F9;padding: 2px;margin-left: 1px;padding-left: 10px;padding-top: 10px;display: none">
    					   	...
    					   	<tr style="height: 30px;">
    					   		<td width="35%">
    					   				<span>
    					   					ชื่อลูกค้าตรวจสอบการติดตั้ง<span style="color:red">*</span>
    					   				</span>
    					   		</td>
    					   		<td width="65%">
    					   				<span style="padding-left: 3px">
    					   					<input type="text"  style="height: 30px;" id="BIT_CUST_NAME" />
    					   				</span>
    					   		</td>
    					   	</tr> 
    					   	<tr style="height: 30px;">
    					   		<td width="35%">
    					   				<span>
    					   					แนบเอกสาร ( 50 Mb.)<span style="color:red">*</span>
    					   				</span>
    					   		</td>
    					   		<td width="65%">
    					   				<span style="padding-left: 3px">
    					   				    <input  id="BIT_DOC_ATTACH" type="button" value="Upload">  
    					   				</span>
    					   	...
    					   	<tr style="height: 30px;">
    					   		...
    					   		<td width="65%">
    					   				<span style="padding-left: 3px">
    					   				 	<input  id="BIT_ATTACH_1" type="button" value="Upload"> 
    					   				</span>
    					   	...
    					   	<tr style="height: 30px;">
    					   		...
    					   		<td width="65%">
    					   				<span style="padding-left: 3px">
    					   					<input  id="BIT_ATTACH_2" type="button" value="Upload"> 
    					   				</span>
    					   	...
    					   	<tr style="height: 30px;">
    					   		...
    					   		<td width="65%">
    					   				<span style="padding-left: 3px">
    					   					<input  id="BIT_ATTACH_3" type="button" value="Upload"> 
    					   				</span>
    					   	...
    					   	<tr style="height: 30px;">
    					   		...
    					   		<td width="65%">
    					   				<span style="padding-left: 3px">
    					   					<input  id="BIT_ATTACH_4" type="button" value="Upload"> 
    					   				</span>
    					   	...
    					   	<tr style="height: 30px;">
    					   		...
    					   		<td width="65%">
    					   				<span style="padding-left: 3px">
    					   					<input  id="BIT_ATTACH_5" type="button" value="Upload"> 
    					   				</span>
    					   	...
    					   	<tr style="height: 30px;">
    					   		...
    					   		<td width="65%">
    					   				<span style="padding-left: 3px">
    					   					<input  id="BIT_ATTACH_6" type="button" value="Upload"> 
    					   				</span>
    					   	...
    					   	<tr style="height: 30px;">
    					   		...
    					   		<tr style="height: 30px;">
    					   		<td width="50%" align="center">
    					   				 V(R)<input type="text" id="BIT_TEST_INPUT_1" style="width:100px;height: 30px;"/>Vac
    					   		</td>
    					   		<td width="50%" align="center">
    					   				 V(R)<input type="text" id="BIT_TEST_OUTPUT_1" style="width:100px;height: 30px;"/>Vac
    					   		</td>
    					   	</tr> 
    					   	<tr style="height: 30px;">
    					   		<td width="50%" align="center">
    					   				 V(S)<input type="text" id="BIT_TEST_INPUT_2"  style="width:100px;height: 30px;"/>Vac
    					   		</td>
    					   		<td width="50%" align="center">
    					   				 V(S)<input type="text" id="BIT_TEST_OUTPUT_2" style="width:100px;height: 30px;"/>Vac
    					   		</td>
    					   	</tr> 
    					   	<tr style="height: 30px;">
    					   		<td width="50%" align="center">
    					   				 V(T)<input type="text" id="BIT_TEST_INPUT_3" style="width:100px;height: 30px;"/>Vac
    					   		</td>
    					   		<td width="50%" align="center">
    					   				 V(T)<input type="text" id="BIT_TEST_OUTPUT_3" style="width:100px;height: 30px;"/>Vac
    					   		</td>
    					   	</tr> 
    					   	<tr style="height: 30px;">
    					   		<td width="50%" align="center">
    					   				 V(g)<input type="text"  id="BIT_TEST_INPUT_4"  style="width:100px;height: 30px;"/>Vac
    					   		</td>
    					   		<td width="50%" align="center">
    					   				 V(g)<input type="text"  id="BIT_TEST_OUTPUT_4" style="width:100px;height: 30px;"/>Vac
    					   		</td>
    					   	</tr> 
    					   	<tr style="height: 30px;">
    					   		<td width="50%" align="center">
    					   				 I(R)<input type="text"  id="BIT_TEST_INPUT_5"  style="width:100px;height: 30px;"/>Amp
    					   		</td>
    					   		<td width="50%" align="center">
    					   				 I(R)<input type="text" id="BIT_TEST_OUTPUT_5" style="width:100px;height: 30px;"/>Amp
    					   		</td>
    					   	</tr> 
    					   	<tr style="height: 30px;">
    					   		<td width="50%" align="center">
    					   				 I(S)<input type="text"   id="BIT_TEST_INPUT_6" style="width:100px;height: 30px;"/>Amp
    					   		</td>
    					   		<td width="50%" align="center">
    					   				 I(S)<input type="text"  id="BIT_TEST_OUTPUT_6" style="width:100px;height: 30px;"/>Amp
    					   		</td>
    					   	</tr> 
    					   	<tr style="height: 30px;">
    					   		<td width="50%" align="center">
    					   				 I(T)<input type="text"   id="BIT_TEST_INPUT_7" style="width:100px;height: 30px;"/>Amp
    					   		</td>
    					   		<td width="50%" align="center">
    					   				 I(T)<input type="text"  id="BIT_TEST_OUTPUT_7" style="width:100px;height: 30px;"/>Amp
    					   		</td>
    					   	</tr> 
    					   	<tr style="height: 30px;">
    					   		<td width="50%" align="center">
    					   		...
    					   		<td width="50%" align="center">
    					   				Load<input type="text" id="BIT_LOAD" style="width:100px;height: 30px;"/>%
    					   		</td>
    					   	</tr> 
    					   	<tr style="height: 30px;">
    					   		<td width="50%" align="center">
    					   		...
    					   		<td width="50%" align="center">
    					   				Backup time<input type="text" id="BIT_BACKUP_TIME"  style="width:100px;height: 30px;"/>min
    					   		</td>
    					   	</tr> 
    					   	<tr style="height: 30px;">
    					   		<td width="100%" colspan="2" align="left">
    					   				 ข้อเสนอแนะ(Comment)&nbsp;&nbsp;&nbsp;<input type="text" id="BIT_COMMENT"  style="width:350px; height: 30px;"/>
    					   		</td> 
    					   	</tr>
            ...
            <c:if test="${isOperationAccount && ( state=='wait_for_operation_delivery' || state=='wait_for_operation_install' ) }">
				<div align="center" style="padding-top: 10px">
				 <table border="0" style="width: 100%">
				 	<tr>
				 		<td width="20%">
				 			<a class="btn btn-info"  onclick="loadDynamicPage('dispatcher/page/delivery_operation?bsoId=${bsoId}&mode=edit&state=${state}&requestor=${requestor}')"><i class="icon-chevron-left icon-white"></i>&nbsp;<span style="color: white;font-weight: bold;">Back</span></a>
				 		</td>
				 		<td width="80%"> 
				 		<div align="center">
				 		 <c:if test="${state=='wait_for_operation_delivery'}">
    					     <a class="btn btn-primary"  onclick="doCloseDeliveryJob('1','wait_for_supervisor_close','${requestor}','1','ข้อมูลถูกส่งไปก่อนหน้านี้ เรียบร้อยแล้ว','ข้อมูลถูกส่งไป Supervisor  เรียบร้อยแล้ว','Sale Order wait for Supervisor Close','1',true)"><i class="icon-ok icon-white"></i>&nbsp;<span style="color: white;font-weight: bold;">Update</span></a>
				 		 </c:if>
				 		 <c:if test="${state=='wait_for_operation_install'}">
				 		    <a class="btn btn-primary"  onclick="doCloseInstallJob('1','wait_for_supervisor_close','${requestor}','1','ข้อมูลถูกส่งไปก่อนหน้านี้ เรียบร้อยแล้ว','ข้อมูลถูกส่งไป Supervisor  เรียบร้อยแล้ว','Sale Order wait for Supervisor Close','1',true)"><i class="icon-ok icon-white"></i>&nbsp;<span style="color: white;font-weight: bold;">Update</span></a>
				 		 </c:if>
				 		 </div>
				 		</td>
				 	</tr>
