...
function assignHOD(){
	...
	//alert(bdeptHOD.length);
	 if(bdeptHOD.length>0){
		var querys=[];  
		var query="update "+SCHEMA_G+".BPM_DEPARTMENT set BDEPT_HDO_USER_ID="+bdeptHOD+" where BDEPT_ID=${brId}"; 
		querys.push(query); 
		SynDomeBPMAjax.executeQuery(querys,{
			callback:function(data){ 
				...
				if(data!=0){
					loadDynamicPage('dispatcher/page/service_management?brId=${brId}&mode=edit');
				}
			}
		});
	}
...
function assignUserToDept(){
	...
	//alert(bdeptUserId.length);
	 if(bdeptUserId.length>0){
		var querys=[];
		var query="insert into "+SCHEMA_G+".BPM_DEPARTMENT_USER set USER_ID="+bdeptUserId+",BDEPT_ID=${brId}";
		querys.push(query); 
		SynDomeBPMAjax.executeQuery(querys,{
			callback:function(data){ 
				if(data.resultMessage.msgCode=='ok'){
...
function getService(){  
	...
	$("#service_title").html("Job Service ("+function_message+")");
  if(isEdit){  
	  var query=" SELECT "+
		" BR_ID,"+
		" BR_NAME,"+
		...
		" BR_CLOSE_DATE ,"+
		" BR_NO "+ // 151-213
		" FROM "+SCHEMA_G+".BPM_REPAIRING  where BR_ID=${brId}";
	   SynDomeBPMAjax.searchObject(query,{
		  ...
		  SynDomeBPMAjax.getRunningNo("SERVICE","ymd","5",{
			  if(data.resultMessage.msgCode=='ok'){
			  ...
			  callback:function(data){ 
				if(data!=null && data.length>0){
					$("#brNo").val(data);
					var querys=[];  
					var query="insert into "+SCHEMA_G+".BPM_REPAIRING set BR_NO='"+data+"'";
					querys.push(query); 
					SynDomeBPMAjax.executeQuery(querys,{
						callback:function(data){
							if(data.resultMessage.msgCode=='ok'){
							...
							if(data!=0){
								query=" SELECT "+
								   " BR_ID , BR_NO FROM "+SCHEMA_G+".BPM_REPAIRING where BR_NO='"+$("#brNo").val()+"'";
								  SynDomeBPMAjax.searchObject(query,{
										callback:function(data2){ 
											...
											loadDynamicPage('dispatcher/page/service_management?brId='+data2[0][0]+'&mode=edit');
										}
								  }); 
...
function doAction(id){
	var querys=[]; 
	var query="DELETE FROM "+SCHEMA_G+".BPM_DEPARTMENT_USER where user_id="+id+" and BDEPT_ID=${brId}"; 
	querys.push(query); 
	SynDomeBPMAjax.executeQuery(querys,{
		callback:function(data){ 
			if(data.resultMessage.msgCode=='ok'){
...
function searchItemList(_page){  
	...
	var query="SELECT "+
		...
		"AMOUNT, "+
		"PRICE "+
		"FROM "+SCHEMA_G+".BPM_SALE_PRODUCT_ITEM where BR_ID=${brId}";
...
function doAssignTeam(){
	...
	bootbox.hideAll();
	doUpdateState('2','wait_for_operation',username_team,'1','ข้อมูลถูกส่งไปก่อนหน้านี้ เรียบร้อยแล้ว','ข้อมูลถูกส่งไป Team เรียบร้อยแล้ว','Job Service wait for Operation','1',true);
}
...
function doUpdateJob(btdl_type,btdl_state,owner,owner_type,message){
	var querys=[]; 
	var query="update "+SCHEMA_G+".BPM_TO_DO_LIST set BTDL_HIDE='0'  where BTDL_REF='${brId}' and "+
	...
	SynDomeBPMAjax.executeQuery(querys,{
			callback:function(data){ 
				...
				if(data!=0){
					 querys=[];  
					 query="update "+SCHEMA_G+".BPM_REPAIRING set BR_STATE='"+btdl_state+"' where BR_ID=${brId}";
					querys.push(query); 
					SynDomeBPMAjax.executeQuery(querys,{
						callback:function(data){ 
							if(data.resultMessage.msgCode=='ok'){
...
function doSendToSupervisor(){
	var selectValue=$("select[id=supervisor_select] option:selected").val();
	 doUpdateState('2','wait_for_assign_to_team',selectValue,'1','ข้อมูลถูกส่งไปก่อนหน้านี้ เรียบร้อยแล้ว','ข้อมูลถูกส่งไป Supervisor เรียบร้อยแล้ว','Job Service wait for assign to Team','1',true);
	//doUpdateState();
}
...
// 1(type จัดส่ง),wait_for_create_to_express,ROLE_QUOTATION_ACCOUNT,2(role),ข้อมูลถูกส่งไปก่อนหน้านี้ เรียบร้อยแล้ว,ข้อมูลถูกส่งไปฝ่าย บัญชี เรียบร้อยแล้ว,Job Service Created,1(show,0=hide)
function doUpdateState(btdl_type,btdl_state,owner,owner_type,message_duplicate,message_created,message_todolist,hide_status,is_hide_todolist){   
	var query="SELECT *  FROM "+SCHEMA_G+".BPM_TO_DO_LIST where BTDL_REF='${brId}' and "+
	"BTDL_TYPE='"+btdl_type+"' and BTDL_STATE='"+btdl_state+"' and BTDL_OWNER='"+owner+"' and BTDL_OWNER_TYPE='"+owner_type+"'  ";
	 SynDomeBPMAjax.searchObject(query,{
			callback:function(data){ 
				if(data.resultMessage.msgCode=='ok'){
				...
				if(data!=null && data.length>0){
					...
					var querys=[];  
					 query="insert into "+SCHEMA_G+".BPM_TO_DO_LIST(BTDL_REF,BTDL_TYPE,BTDL_STATE,BTDL_OWNER,BTDL_OWNER_TYPE,BTDL_MESSAGE,"+
							...
							"('${brId}','"+btdl_type+"','"+btdl_state+"','"+owner+"','"+owner_type+"','"+message_todolist+"','',now(),	null,'"+hide_status+"','${username}') ";
					 if('${state}'!='' && is_hide_todolist){
					  query2="update "+SCHEMA_G+".BPM_TO_DO_LIST set BTDL_HIDE='0'  where BTDL_REF='${brId}' and "+
...
function doSaveDraftAction(){  
	loadDynamicPage('dispatcher/page/service_search');
   <%-- 
	...
	if($("#mode").val()=='edit'){
		 query="update "+SCHEMA_G+".BPM_DEPARTMENT set BDEPT_NAME=?, BDEPT_DETAIL=?,BDEPT_HDO_USER_ID="+bdeptHOD_str+" where BDEPT_ID=${brId}";
	}
	querys.push(query); 
	var list_values=[];
		var values=[];
    ...
    <div style="border: 1px solid #FFC299;background: #F9F9F9;padding: 10px">
	    <form id="breakdownForm" name="breakdownForm"  class="well" action="" method="post">
			...
			<!--  <form class="well"> -->
			 ...
			 <input type="hidden" name="brId" id="brId"  value="${brId}"/> 
            ...
            <input type="hidden" id="bdeptUserId" name="bdeptUserId"/>
			 ...
			 <!--  <pre  class="prettyprint" style="font-family: sans-serif;font-size:12px:;margin-top: 0px"> -->
			  <div align="left">
           	 <strong id="service_title"></strong><input type="text" id="brNo" style="height: 30px;width: 125px" readonly="readonly"/> <br></br>
            	</div>
            	<%-- 
    					   	...
    					   	<tr>
    					   		...
    					   		<td width="80%">
    					   			<span style="padding-left: 3px">
    					   				...
    					   				<input type="checkbox" id="bsoTypeCheck_2" onclick="bsoTypeCheck('2')"/>ติดตั้ง
    					   			</span>
    					   		</td>
    					   	</tr>
    					   </table>
             ...
             <c:if test="${isCallCenter}">
    			<div align="center" style="padding-top: 10px">
    			<table border="0" style="width: 100%">
				 	<tr>
				 		<td width="20%">
				 			<a class="btn btn-info"  onclick="loadDynamicPage('dispatcher/page/service_search')"><i class="icon-chevron-left icon-white"></i>&nbsp;<span style="color: white;font-weight: bold;">Back</span></a>
				 		</td>
				 		<td width="80%">
    					 <a class="btn btn-primary"  onclick="doUpdateState('2','wait_for_send_to_supervisor','ROLE_KEY_ACCOUNT','2','ข้อมูลถูกส่งไปก่อนหน้านี้ เรียบร้อยแล้ว','ข้อมูลถูกส่งไป Key Account เรียบร้อยแล้ว','Job Service Created','1',true)"><i class="icon-ok icon-white"></i>&nbsp;<span style="color: white;font-weight: bold;">ส่งไป Key Account</span></a>
				...
				<div align="center" style="padding-top: 10px">
				 <table border="0" style="width: 100%">
				 	<tr>
				 		...
				 		<td width="80%">
				 		 <a class="btn btn-primary"  onclick="doUpdateState('2','wait_for_quotation','ROLE_QUOTATION_ACCOUNT','2','ข้อมูลถูกส่งไปก่อนหน้านี้ เรียบร้อยแล้ว','ข้อมูลถูกส่งไปรอเสนอ Quotation เรียบร้อยแล้ว','Job Service wait for Quotation','1',false)"><i class="icon-ok icon-white"></i>&nbsp;<span style="color: white;font-weight: bold;">ส่งไปเสนอ Quotation</span></a>
			...
			<c:if test="${isQuotationAccount && state=='wait_for_quotation'}">
				<div align="center" style="padding-top: 10px">
				 <table border="0" style="width: 100%">
				 	<tr>
				 		<td width="20%">
				 		...
				 		<td width="80%">
    					  <div align="center"><a class="btn btn-primary"  onclick="doUpdateJob('2','wait_for_quotation','ROLE_QUOTATION_ACCOUNT','2','Job Service updated ')"><i class="icon-ok icon-white"></i>&nbsp;<span style="color: white;font-weight: bold;">Update Status</span></a>
    					  </div>
				 		</td>
				 	</tr>
				 </table> 
			...
			<c:if test="${isSupervisorAccount && state=='wait_for_assign_to_team'}">
				<div align="center" style="padding-top: 10px">
				 <table border="0" style="width: 100%">
				 	<tr>
				 		...
				 		<td width="80%">
				 			<a class="btn btn-primary"  onclick="doUpdateState('2','wait_for_stock','ROLE_STOCK_ACCOUNT','2','ข้อมูลถูกส่งไปก่อนหน้านี้ เรียบร้อยแล้ว','ข้อมูลถูกส่งไป Store เรียบร้อยแล้ว','Job Service wait for Store','1',false)"><i class="icon-ok icon-white"></i>&nbsp;<span style="color: white;font-weight: bold;">ส่งไป Store</span></a>
			 ...
			 <c:if test="${isStockAccount && state=='wait_for_stock'}">
				<div align="center" style="padding-top: 10px">
				 <table border="0" style="width: 100%">
				 	<tr>
				 		<td width="20%">
				 		...
				 		<td width="80%">  
    					 <div align="center"> <a class="btn btn-primary"  onclick="doUpdateJob('2','wait_for_stock','ROLE_STOCK_ACCOUNT','2','Job Service updated ')"><i class="icon-ok icon-white"></i>&nbsp;<span style="color: white;font-weight: bold;">Close Job</span></a></div>
				 		</td>
				 	</tr>
				 </table>
				</div>
				...
				<div align="center" style="padding-top: 10px">
				 <table border="0" style="width: 100%">
				 	<tr>
				 		...
				 		<td width="80%"> 
    					 <div align="center"> <a class="btn btn-primary"  onclick="doUpdateState('2','wait_for_supervisor_close','${requestor}','1','ข้อมูลถูกส่งไปก่อนหน้านี้ เรียบร้อยแล้ว','ข้อมูลถูกส่งไป Supervisor  เรียบร้อยแล้ว','Job Service wait for Supervisor Close','1',true)"><i class="icon-ok icon-white"></i>&nbsp;<span style="color: white;font-weight: bold;">Close Job</span></a></div>
				 		</td>
				 	</tr>
				 </table>
				</div>
				...
				<div align="center" style="padding-top: 10px">
				 <table border="0" style="width: 100%">
				 	<tr>
				 		...
				 		<td width="80%"> 
    					 <div align="center"> <a class="btn btn-primary"  onclick="doUpdateState('2','wait_for_keyaccount_close','ROLE_KEY_ACCOUNT','2','ข้อมูลถูกส่งไปก่อนหน้านี้ เรียบร้อยแล้ว','ข้อมูลถูกส่งไป Key Account  เรียบร้อยแล้ว','Job Service wait for Key Account Close','1',true)"><i class="icon-ok icon-white"></i>&nbsp;<span style="color: white;font-weight: bold;">Close Job</span></a></div>
				 		</td>
				 	</tr>
				 </table>
				</div>
				...
				<div align="center" style="padding-top: 10px">
				 <table border="0" style="width: 100%">
				 	<tr>
				 		...
				 		<td width="80%"> 
    					 <div align="center"> <a class="btn btn-primary"  onclick="doUpdateJob('2','wait_for_keyaccount_close','ROLE_KEY_ACCOUNT','2','Job Service Closed')"><i class="icon-ok icon-white"></i>&nbsp;<span style="color: white;font-weight: bold;">Close Job</span></a></div>
				 		</td>
				 	</tr>
				 </table>
				</div>
