...
function getSolution(){ 
  ...
  if(isEdit){
	var query="SELECT BSL_ID,BSL_NAME  FROM "+SCHEMA_G+".BPM_SOLUTION where BSL_ID=${bslId}";
...
function doSolutionAction(){
	...
	var querys=[];
	var query="insert into "+SCHEMA_G+".BPM_SOLUTION set BSL_NAME=?  ";
	if($("#mode").val()=='edit'){ 
		 query="update "+SCHEMA_G+".BPM_SOLUTION set BSL_NAME=? where BSL_ID=${bslId}";
	}
	querys.push(query);
	...
	SynDomeBPMAjax.executeQueryWithValues(querys,list_values,{
			callback:function(data){ 
				...
				if(data!=0){
					loadDynamicPage("setting/page/solutionType_search");
				}
			}
		});
  }
    ...
    <div style="border: 1px solid #FFC299;background: #F9F9F9;padding: 10px">
	    <form id="breakdownForm" name="breakdownForm"  class="well" action="" method="post">
			...
			<!--  <form class="well"> -->
			 ...
			 <input type="hidden" name="bsId" id="bsId"  value="${bslId}"/> 
			 ...
			 <!--  <pre  class="prettyprint" style="font-family: sans-serif;font-size:12px:;margin-top: 0px"> -->
			  <div align="left">
			    ...
			    <table border="0" width="100%" style="font-size: 12px">
    				...
    				<tr valign="middle">
    					<td width="25%" align="right"><span style="font-size: 13px;padding: 15px">วิธีแก้ไข/ป้องกันปัญหา :</span></td>
    			...
    			<div align="center" style="padding-top: 10px">
						<a class="btn btn-info"  onclick="loadDynamicPage('setting/page/solutionType_search')"><i class="icon-chevron-left icon-white"></i>&nbsp;<span style="color: white;font-weight: bold;">Back</span></a>	
    					 <a class="btn btn-primary"  onclick="doSolutionAction()"><i class="icon-ok icon-white"></i>&nbsp;<span style="color: white;font-weight: bold;">Submit</span></a>
						</div>
			  </form>  
